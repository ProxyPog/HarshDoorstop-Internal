...
void ESP::RenderText(SDK::ACharacter* pawn, std::string& text, ImColor color)
{
	if (!pawn || !Config::m_pMyController || Validity::IsBadPoint(Config::m_pMyController))
		return;

	SDK::FVector actorLocation = pawn->K2_GetActorLocation();
	SDK::FVector2D screenPosition;

	if (!Config::m_pMyController->ProjectWorldLocationToScreen(actorLocation, &screenPosition, false))
		return;
	...
	// Draw text centered horizontally above player
	ImVec2 textSize = ImGui::CalcTextSize(text.c_str());
	ImVec2 pos = ImVec2(screenPosition.X - textSize.x / 2.0f, screenPosition.Y - textSize.y - 15.0f); // 15px above head
